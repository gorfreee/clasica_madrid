...
`auditorio-nacional`, `fundacion-juan-march`, `teatro-zarzuela`, `cndm` y `real-hermandad-refugio` forman parte del `all` programado. March, Zarzuela, Auditorio y CNDM salen por el fetch relay (`useFetchRelay`). Refugio marca `fetchTransport: 'direct-then-relay'`: el archivo HTML se pide primero en directo y el Worker sólo si ese hop se bloquea. Ver [infra/fetch-relay](../infra/fetch-relay/README.md). El Worker se despliega con [deploy-fetch-relay.yml](../.github/workflows/deploy-fetch-relay.yml) o desde el Dashboard; añadir otra fuente al relay es `useFetchRelay: true` en el registry. Las peticiones a `/wp-json/` envían `Accept: application/json` sin `text/html` ni `*/*`. Un HTTP 202 de captcha no se acepta como documento; el Worker puede reintentarlo si hay cookie nueva o cuerpo `sgcaptcha`. Refugio usa el archivo oficial `/categoria-eventos/conciertos/` (no `/conciertos/`); REST queda como fallback secundario. Zarzuela mantiene pacing de origen, `Retry-After` y un circuito por fichas distintas.
