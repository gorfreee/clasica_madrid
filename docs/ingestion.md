...
- Un adapter puede descartar un candidato ya reconocido (p. ej. Madrid Datos: recurrence no soportada, falta título/URL/id/fecha/lugar). Esos descartes son diagnóstico (`adapterDiscards` en el summary y, si hay observabilidad, journal `adapter-discard`): no cambian eligibility, health ni publicación. No se cuentan objetos fuera de ámbito (p. ej. exposiciones en Madrid Datos). La hora ausente en Madrid Datos ya no descarta: la occurrence conserva la fecha y `time` queda `null`.
