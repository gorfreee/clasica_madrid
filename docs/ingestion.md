...
- Clasificación/enrichment: reglas deterministas y knowledge siempre preceden a la IA. Los purposes son `eligibility`, `composer-extraction`, `access-classification` y `taxonomy`. Eligibility sólo usa IA si el determinista deja `uncertain`; un `include` o `exclude` determinista no se reabre. En un `include`, composers y access sólo usan fallback si siguen sin resolver y existe texto observado suficiente. Después se recalculan `eras` por knowledge a partir de compositores/obras observados. Taxonomy puede completar `formats` si siguen vacíos; no rellena `eras`. Una respuesta de taxonomía sin `formats` no cuenta como resolución del formato: se reutiliza el fallback de modelos. Los vacíos/`unknown` siguen siendo resultados válidos y cualquier fallo técnico conserva el include y los valores deterministas.
...
- `eligibility`: sólo para casos deterministas `uncertain`.
- `composer-extraction`: sólo cuando structured composers, `works[].composerName` y knowledge no resolvieron nada, y `programText` observado tiene estructura de programa suficiente. El modelo propone nombre + span; el código exige que ambos aparezcan en el texto, rechaza performers/roles y deduplica/canonicaliza antes de aceptar. El evento enriquecido vuelve a pasar por el resolver determinista de `eras`.
...
- `taxonomy`: sólo para un include con `eras` o `formats` todavía vacíos; no reabre eligibility.
