...
describe('buildPlaceAddress', () => {
  ...
  it('abre la ubicación en Google Maps e incluye el municipio en la consulta', () => {
    ...
    expect(address).toEqual({
      ...
      locality: null,
      mapsUrl:
        'https://www.google.com/maps/search/?api=1&query=Pr%C3%ADncipe%20de%20Vergara%2C%20146%2C%20Madrid',
      accessibleLabel:
        'Ver Príncipe de Vergara, 146 en Google Maps. Se abre en una pestaña nueva.',
    });
  });
  ...
  it('no duplica el municipio en la consulta si la calle ya lo nombra', () => {
    ...
    expect(address?.mapsUrl).toBe(
      'https://www.google.com/maps/search/?api=1&query=Plaza%20de%20Isabel%20II%2C%20s%2Fn%2C%2028013%20Madrid',
    );
...
describe('dirección en fichas', () => {
  it('la ficha de evento enlaza la dirección del lugar principal', () => {
    ...
    expect(page?.placeAddress?.mapsUrl).toContain('google.com/maps/search');
  });
  ...
  it('un lugar sin calle no ofrece un enlace de ubicación', () => {
