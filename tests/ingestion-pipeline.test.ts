...
describe('aislamiento de fallos por fuente', () => {
  it('continúa con las fuentes sanas si una falla', async () => {
    ...
    expect(run.summary.sourcesFailed.map((item) => item.sourceId)).toEqual(['teatro-real', 'patrimonio-nacional']);
  ...
  it('si fallan todas las fuentes el lote no se escribe y el exit code no es exitoso', async () => {
    ...
    expect(run.summary.sourcesFailed.map((item) => item.sourceId)).toEqual([
      'auditorio-nacional', 'teatro-real', 'madrid-datos', 'teatro-zarzuela', 'fundacion-juan-march', 'fundacion-orcam', 'orquesta-coro-rtve', 'teatros-canal', 'fundacion-canal', 'circulo-bellas-artes', 'cndm', 'basilica-san-miguel', 'fundacion-piu-mosso', 'real-hermandad-refugio', 'real-academia-bellas-artes', 'fundacion-goethe', 'madrid-a-tempo', 'patrimonio-nacional',
    ]);
    expect(run.summary.written).toEqual([]);
