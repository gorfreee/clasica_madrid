...
describe('estructura de filtros avanzados', () => {
  ...
  it('distingue el estado activo de los atajos del hover y el foco', () => {
    ...
    expect(activeBlock).not.toContain('background: var(--color-text)');
    ...
    expect(hoverMedia).toContain('background: var(--color-text)');
    expect(hoverMedia).toContain('.filters .filters__toggle:hover');
    expect(touchMedia).toContain('.filters .shortcut:hover');
    expect(touchMedia).toContain('background: var(--color-surface-veil)');
