...
- Ingestion v3 phase 1 lives in `src/ingestion/` (`ingest:sync`, `ingest:source`). Phase 2.1 adds optional `hydrate` on adapters and detail parsers in `src/ingestion/detail/`; a detail-page failure is event-local. Phase 2.2 adds the deterministic classifier in `src/ingestion/classification/` and a small composer knowledge base in `src/ingestion/knowledge/`. The classifier is tested against the golden set; it is **not** yet the publication gate of `runIngest` (phase 2.4). Candidate JSON + `ingest:promote` remains as a manual/legacy path. Working directories under `ingestion/inbox`, `work` and `rejected` are gitignored. Batch writes use a short-lived `.ingest-tmp-*` directory inside the data tree (also gitignored). The ingest CLI rejects unknown flags and `--data-dir` without a value. Do not remove that infrastructure unless a task says so. Do not implement later v3 phases (AI fallback, pipeline publication gate, GitHub Actions, auto-merge, discovery agents) unless a task asks for that phase.
