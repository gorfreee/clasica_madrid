...
- una obra de concierto de tango/nuevo tango (p. ej. un programa de piezas concertísticas, no una milonga bailable ni un espectáculo de baile) NO es pop ni música de cine por el género. Sin identidad excluida positiva, no uses exclude.
...
Una mención secundaria de charla, conferencia o «charlas previas» / «charla sobre el concierto» en la ficha de un concierto real NO es la identidad del evento: el concierto sigue siendo el concierto. Exclude de actividad no interpretativa sólo si el título o la categoría identifican ESTA ficha como charla/taller/conferencia.

Reglas:
- precisión > cobertura;
- uncertain es una salida válida y preferible a inventar certeza;
- exclude exige una identidad excluida positiva (pop, cine, jazz como ciclo, flamenco musical, danza-espectáculo, taller/charla como actividad principal, etc.). La mera ausencia de programa obra-por-obra NO es exclude: si no basta para include → uncertain;
...
- no uses «correo electrónico» ni prosa de formularios como evidencia musical;
- no expliques el razonamiento paso a paso; no escribas prosa fuera del JSON.
